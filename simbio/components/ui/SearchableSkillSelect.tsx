...
export function SearchableSkillSelect({
  ...
  customSkillName = '',
  onSelectSkill,
  placeholder = 'Cari atau pilih skill...',
  ...
  return (
    <div ref={containerRef} className="relative w-full space-y-2">
      {/* Dropdown Selector Button (Soft-UI Styled) */}
      <button
        type="button"
        onClick={() => setIsOpen(!isOpen)}
        className="w-full px-4 py-2.5 bg-slate-50 text-slate-900 font-medium text-xs rounded-xl border border-slate-200 flex items-center justify-between shadow-2xs hover:bg-white hover:border-[#FF6B30] transition"
      >
        <span className="truncate">
          {isOtherSelected
            ? `Lainnya: ${otherInput || 'Skill Custom'}`
            : selectedSkill
        ...
        <div className="p-3 bg-orange-50/70 rounded-xl border border-orange-200/80 space-y-2 shadow-2xs">
          ...
          <div className="flex gap-2">
            <input
              ...
              placeholder="Contoh: Quantum Computing, Pottery..."
            ...
            <button
              ...
              className="soft-button text-xs px-3 py-2 flex items-center gap-1 disabled:opacity-50"
            >
              <Plus className="w-3.5 h-3.5" />
              <span>{creatingCustom ? 'Menambah...' : 'Tambah'}</span>
            </button>
          </div>
        </div>
      )}
      ...
      {isOpen && (
        <div className="absolute top-full left-0 right-0 mt-2 z-50 bg-white border border-slate-200 rounded-2xl shadow-lg p-3 space-y-3 max-h-72 flex flex-col">
          ...
          <button
            ...
            className={`w-full text-left p-2.5 rounded-xl border text-xs font-bold flex items-center gap-2 transition ${
              isOtherSelected
          ...
          >
            ...
            <span>+ Skill Lainnya / Kustom (Tulis Sendiri)</span>
          </button>
        </div>
      )}
    </div>
